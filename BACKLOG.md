...
| T-130 | O painel ganha tema (django-jazzmin repintado com os tokens da SPEC-014), rótulos em pt-BR, e um dashboard que responde "rodou hoje?" em vez de listar tabelas; **e passa a ser alcançável pelo domínio** — o `./scripts/prod.sh nginx` não roteava nem o caminho do painel nem `/static/`, então a URL abria a landing (200) e o whitenoise nunca era alcançado | 018/014 | done |
...
| T-113 | `flexao` e `abdominal` para `validado` (migration 0019) — visíveis para anônimo e Free | 020/018 | **feito** (2026-08-17) — decisão de produto. A flexão tem lastro (MAE 0,20 rep, T-111); o **abdominal não tem vídeo nenhum de gente real** e vai por decisão, com o risco registrado na migration e a dívida ainda em `SEM_MATERIAL_REAL`. Freio de mão: campo Maturidade no painel |
| T-112 | Trava de confirmação da variação: exercício com duas montagens de cena não passa do "Ligar câmera" sem alguém dizer qual delas vale — cards responsivos com as duas visíveis, checkbox "não mostrar novamente" desmarcado por padrão, dispensa por exercício no armazenamento, e o card da coluna como caminho permanente | 020/014/015 | **feito** (2026-08-17) — nasce do risco de produto: vista errada não conta pior, conta **zero**, e zero é lido como "o app é ruim". Cobre também quem troca de exercício com a câmera já ligada |
...
| T-139 | Cadência vira o eixo de progresso: relatório da série mostra reps/min sempre e tempo até a meta no modo contado; comparação por exercício nas últimas 4 semanas como **derivação pura** sobre `SessionResult` (sem tabela, sem contador). Depende de T-134 | 023/010/017 | **feito** (2026-08-19) — achou dois espelhos de contrato para trás: `SessionReport` sem os 4 campos da T-134 e `SessionEndReason` sem `TARGET_REACHED` (toda série que batesse a meta seria anunciada como "Sessão encerrada") |
...
| T-163 | **`robots.txt` + `sitemap.xml` + descoberta.** Ambos gerados da tabela de rotas no build; `sitemap` com `<xhtml:link>` de idioma alternativo por URL; `robots.txt` liberando o site e apontando o sitemap. **O `/app/` NÃO é bloqueado** — a redação original pedia isso e o meio estava errado: `Disallow` impede o rastreamento, o robô nunca lê o `noindex`, e a URL pode ser listada assim mesmo por ser linkada do site (ver DEVLOG). Decisão a registrar na task: política para rastreadores de LLM (GPTBot, ClaudeBot, PerplexityBot) — recomendação é **permitir**, porque parte da descoberta de produto hoje acontece dentro de um chat, e nenhum deles executa o bundle. Depende de T-158 *(Tam: M)* | 026 | **feito** (2026-08-18) |
| T-164 | **Open Graph + JSON-LD.** `og:*` e `twitter:card` por rota e por idioma, com imagem própria; JSON-LD `SoftwareApplication` + `Organization`. Hoje o link compartilhado mostra caixa cinza sem título e sem imagem — e o WhatsApp é o canal que motiva a task, então o critério de aceite é verificado num WhatsApp real, não só no validador. Depende de T-159 *(Tam: M)* | 026/014 | **feito** (2026-08-18) — pendente: conferir o card num WhatsApp real após deploy |

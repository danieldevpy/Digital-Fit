...
class PushUpAnalyzer:
    ...
    def features(self, frame: NormFrame) -> Features:
        ...
        # A referência só CRESCE. Uma sessão que começa com a pessoa no fundo da flexão adota
        # o fundo como prancha, lê `depth` 1.0 e não conta essa primeira subida — perde uma
        # repetição. É a troca deliberada: o contrato em `base.py` diz que fase adotada por
        # palpite vira repetição fantasma, e rep fantasma é pior que rep perdida. Da segunda
        # em diante a referência já é a prancha de verdade.
        if plank_height > self._plank_height:
            self._plank_height = plank_height

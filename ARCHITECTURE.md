...
| 013 | **O conteúdo do site entra no build por snapshot exportado, não por leitura do banco** — `manage.py export_site_catalog` escreve `web/src/site/exercicios.json` a partir de `config.exercises_for(None)`, e o passo roda entre a migration e o `compose build` no `scripts/prod.sh` (SPEC-026, T-165) | a redação original da spec dizia "o pré-render lê o ORM no build", e isso não é executável aqui: o `web.Dockerfile` constrói num `node:22-alpine` sem Django e sem Postgres, e a ordem do deploy é build → migrate → start — na hora do build não há ORM nem API de pé. Das três saídas, a exportação é a única que preserva o que já valia: (a) o build continua **hermético**, então roda no CI e em clone novo sem banco; (b) o conteúdo publicado fica **no diff**, e dá para saber o que estava no ar em cada deploy; (c) banco fora do ar **congela** o conteúdo em vez de derrubar o build. Buscar da API por HTTP exigiria reordenar o deploy e tornaria o build dependente de rede; renderizar as páginas no Django abandonaria o `renderToString` e daria ao mesmo site duas máquinas de render, contra a ADR-012. Quem entra continua decidido num lugar só (`exercises_for`), então "some do app" e "some do site" são a mesma edição | conteúdo que precise mudar sem deploy (o preço declarado é que exercício despublicado só some do site no próximo build), ou volume de páginas que torne o rebuild caro demais — o mesmo gatilho da ADR-012, e os dois se movem juntos |
